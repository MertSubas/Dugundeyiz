    ...
    function popupLoaded(init) {
        ...
        if (options.pagePopup) {
            ...
            iframe.css({
                ...
                width: !popupOverflowed ? (popup[0].offsetParent ? popup[0].offsetParent.clientWidth : $(document.body).width()) : '',
            })
        } else {
            iframe.css({
                position: 'relative',
...
$(function () {
    ...
    (function () {
        var extractYoutubeId = function (url) {
            ...
            if (!id && /(?:^|")(?:https?:\/\/|)(?:www\.|music\.)?youtube\.com\/(?:embed\/|shorts\/|watch\?v=)(.+?)(?:[\?&].*)?(?:"|$)/i.test(url)) {
                id = RegExp.$1
            }
            if (id) {
        ...
        var updateMenuPosition = function () {
            ...
            if (collapser.css('display') === 'none') {
                ...
                if (ulMaxW && wWidth > ulMaxW + 40) {
                    var align = 'left';
                    if ($(elem[0]).hasClass('wb-menu-mobile-left'))
                        align = 'left';
                    if ($(elem[0]).hasClass('wb-menu-mobile-center'))
                        align = 'center';
                    if ($(elem[0]).hasClass('wb-menu-mobile-right'))
                        align = 'right';
                    ul.css({
                        left: align === 'left' ? box.left : (align === 'right' ? 'unset' : (box.left + box.width / 2 - ulMaxW / 2)),
                        right: align === 'left' ? 'unset' : (align === 'right' ? (wWidth - (box.left + box.width)) : 'unset'),
                        width: '',
                    });
                    var ulBox = ul.get(0).getBoundingClientRect();
                    if (ulBox.right + 20 > wWidth) {
                        ul.css('left', parseInt(ul.css('left')) - (ulBox.right - wWidth) - 20)
                    }
                    if (ulBox.left < 20) {
                        ul.css('left', 20)
                    }
    ...
    (function () {
        ...
        $('.wb-accordion [data-toggle="collapse"]').on('shown.bs.collapse', function (e) {
            var elem = $(e.target);
            elem.trigger('resize')
        })
    }
